...
- **The layout template library** — one render function per layout type (title, agenda, section, content, two-col, stat, quote, split, chart, comparison, process, icon-grid, timeline, closing), each with hard-coded grid, spacing, and typography matching the existing Whatfix design system (colors, Aeonik/DM Sans font stack, dark/light sandwich rules) — moved from "CSS the LLM must reproduce correctly every time" to "CSS that exists exactly once and is always correct."
...
- **Font strategy unchanged in principle, tightened in practice**: PPTX text continues to use DM Sans (already confirmed natively available in Google Slides, avoiding the silent-substitution/reflow problem the research flags as the most common cross-platform fidelity break). No font embedding is introduced in this phase (see Non-goals).
...
The existing Whatfix color palette, typography scale, dark/light sandwich rules, and layout-variety rules from the current skill file (lines 95–164) carry over **unchanged in substance** — this spec doesn't propose new visual design, it proposes moving enforcement of the existing design system from "the LLM must remember and correctly apply these rules to hand-written CSS every time" to "these rules are the only options the renderer exposes." The action-title / one-governing-thought-per-slide content rule likewise carries over as a schema field (e.g. a required, complete-sentence `title` per slide) rather than a prose reminder.
