...
- `{ "type": "image", "x", "y", "w", "h", "brandImage"? (a key from the Brand Graphics list below, e.g. `"logo-dark"` — NOT a `src` path, and NOT an extension: `brandImagePath` derives `.svg` vs `.png` itself), "deckAsset"? (a full filename from `client/public/deck-assets/`, e.g. `"slide-42-image-1.png"`) }` — set exactly one of `brandImage`/`deckAsset`, never a `src` field.
